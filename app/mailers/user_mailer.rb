class UserMailer < ActionMailer::Base
  default :from => "Dahlia Rosa - Vendas <vendas@dahliarosa.com.br>"
 ...
 #Envia email (instruções para recuperar a senha)
  ...
  def order_completed(order)
    @order = order
    @order_itens = OrderProduct.all(:conditions => ['order_id = ?', @order.id])    
    mail(:to=>"vendas@dahliarosa.com.br", :bcc => "rmatuoka@korewa.com.br", :subject => @order.user.nome + " - Pagamento Efetuado")    
  end
  ...
  def user_commented(user, product)
    ...
    mail(:to=>"sac@dahliarosa.com.br", :bcc => "rmatuoka@korewa.com.br", :subject => @user.nome + " comentou no produto " + @product.name)    
  end
  
  def reports(vendasok)
     @vendasefetuadas = vendasok
     mail(:to => "vendas@dahliarosa.com.br", :bcc => "rmatuoka@korewa.com.br", :subject => "Relatorio de Vendas aprovadas!")
   end
